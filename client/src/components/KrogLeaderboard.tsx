  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* My Stats */}
        {myStats && (
          <div style={{ padding: '16px 20px', borderBottom: '1px solid #333', backgroundColor: '#222' }}>
            <div style={{ fontSize: '0.85rem', color: '#888', marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
              {language === 'en' ? 'Your Stats' : 'Din statistikk'}
            </div>
            <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', marginBottom: '12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <span style={{ fontSize: '1.1rem' }}>📖</span>
                <span style={{ color: '#ddd' }}>{language === 'en' ? 'Views' : 'Visninger'}: <strong style={{ color: '#81b64c' }}>{myStats.explanations_viewed}</strong></span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <span style={{ fontSize: '1.1rem' }}>📤</span>
                <span style={{ color: '#ddd' }}>{language === 'en' ? 'Shares' : 'Delinger'}: <strong style={{ color: '#81b64c' }}>{myStats.explanations_shared}</strong></span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <span style={{ fontSize: '1.1rem' }}>🎯</span>
                <span style={{ color: '#ddd' }}>R-Types: <strong style={{ color: '#9b59b6' }}>{myRtypeCount}/15</strong></span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <span style={{ fontSize: '1.1rem' }}>🏅</span>
                <span style={{ color: '#ddd' }}>{language === 'en' ? 'Rank' : 'Rangering'}: <strong style={{ color: '#f1c40f' }}>#{myRank || '-'}</strong></span>
              </div>
            </div>
            {myBadges.length > 0 && (
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ color: isCurrentUser ? '#81b64c' : '#ddd', fontWeight: isCurrentUser ? 600 : 400 }}>
                        {entry.username || 'Unknown'}
                        {isCurrentUser && <span style={{ marginLeft: '6px', fontSize: '0.8rem', color: '#888' }}>({language === 'en' ? 'you' : 'deg'})</span>}
                      </div>
                      {entryBadges.length > 0 && (
                        <div style={{ marginTop: '4px' }}>
                          {entryBadges.map((badge, bidx) => (
                            <span key={bidx} style={{ marginRight: '6px', fontSize: '0.85rem' }} title={badge.name}>
                              {badge.icon}
                            </span>
                          ))}
                        </div>
                    ...
                    <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                      {activeTab === 'views' && (
                        <div style={{ color: '#81b64c', fontWeight: 600, fontSize: '1.1rem' }}>
                      ...
                      {activeTab === 'rtypes' && (
                        <div style={{ color: '#9b59b6', fontWeight: 600, fontSize: '1.1rem' }}>
                          {rtypeCount}/15
                        </div>
                      )}
                    </div>
        ...
        <div style={{ padding: '12px 20px', borderTop: '1px solid #333', backgroundColor: '#222' }}>
          <div style={{ textAlign: 'center', color: '#666', fontSize: '0.85rem' }}>
            {language === 'en'
              ? 'Click moves in game history to learn KROG formulas'
              : 'Klikk på trekk i spillhistorikken for å lære KROG-formler'}
          </div>
        </div>
      </div>
    </div>
