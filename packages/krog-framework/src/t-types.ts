...
export const TTypeDefinitions: Record<TType, TTypeDefinition> = {
  T1: {
    id: 'T1',
 ...
 */
export function getTTypeDefinition(ttype: TType): TTypeDefinition {
 ...
 */
export function getAllTTypes(): TTypeDefinition[] {
 ...
 */
export function getTTypeCapabilities(ttype: TType): TTypeCapabilities {
...
export function determineTType(constraints: {
  ...
  if (constraints.mustAct) return 'T5';
  if (constraints.mustBePassive) return 'T6';
  if (constraints.mustPrevent) return 'T7';
  if (constraints.canInitiate === false) return 'T4';
  if (constraints.mustEngage) return 'T3';
  if (constraints.canRefuse === false) return 'T2';
  return 'T1';
}
